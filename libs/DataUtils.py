...
def DeleteSessionFiles(sessionname):
    ...
    deleteAFile(DSMVERSIONFILE+sessionname)
...
def StoreDSMVersion(version,sessionname):
    CheckAndCreateDataFolderIfNeeded()
    SessionFileName = DSMVERSIONFILE+sessionname
    deleteAFile(SessionFileName)
    text_file = open(SessionFileName, "w+")
    text_file.write(str(version))
    text_file.close()

def GetDSMVersion(sessionname):
    SessionFileName = DSMVERSIONFILE+sessionname
    try:
        ...
        print("Cannot get Version: Session does not exist.")
        exit(1)
